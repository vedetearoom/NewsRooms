...
export function TimerDisplay({ startTime }: { startTime: number }) {
...
export interface InboxTextListCardProps {
  article: RawArticle;
  isSelected: boolean;
  isPending: boolean;
  runningJob?: { startTime: number; jobId: string; name: string };
  onToggleSelect: (id: number) => void;
  onProcessSingle: (id: number) => void;
  onDelete: (id: number) => void;
}

export function InboxTextListCard({
  article,
  isSelected,
  isPending,
  runningJob,
  ...
  return (
    <div
      ...
      className={cn(
        "group flex items-center gap-4 py-2.5 border-b border-zinc-100 dark:border-white/[0.04] hover:bg-zinc-50/80 dark:hover:bg-white/[0.02] transition-colors cursor-pointer"
      )}
    >
      {/* Left: Checkbox */}
      ...
      <div className="flex-1 min-w-0 pr-4">
        <p className={cn(
          ...
          article.is_processed
            ? "text-zinc-500 font-normal dark:text-zinc-400 group-hover:text-zinc-700 dark:group-hover:text-zinc-300"
            : "text-zinc-900 font-medium dark:text-zinc-100"
        )}>
          {article.title || t("pipeline.untitled", "Untitled")}
        </p>
        <p className={cn(
          "text-[11.5px] mt-px truncate transition-colors",
          article.is_processed
      ...
      {/* Right: Hover Replacement Actions */}
          ...
          return (
            <>
              ...
              <div className="absolute right-0 flex items-center gap-4 transition-transform duration-200 group-hover:translate-x-[150%]">
                <span className={cn(
                  "w-[64px] text-right text-[11.5px] tabular-nums transition-colors",
                  article.is_processed
                    ? "text-zinc-400 dark:text-zinc-600"
                    : "text-zinc-500 dark:text-zinc-400"
                )}>
                ...
                <div className="flex items-center justify-end w-[72px]">
                  {article.is_processed ? (
                    <span className="text-[11px] text-zinc-400 dark:text-zinc-500 bg-zinc-50 dark:bg-[#15161a] border border-zinc-200/50 dark:border-white/[0.05] px-1.5 py-0.5 rounded whitespace-nowrap">
              ...
              <div className="absolute right-0 flex items-center gap-1 transition-transform duration-200 translate-x-[150%] group-hover:translate-x-0 w-full justify-end bg-gradient-to-l from-zinc-50 via-zinc-50 to-transparent dark:from-[#0f1013] dark:via-[#0f1013] pl-8">
                {isPending && (
                  <button
                    onClick={e => { e.stopPropagation(); onProcessSingle(article.id); }}
                    ...
                    title={t("pipeline.processArticle", "Process this article")}
                  >
                ...
                {article.url && (
                  <a href={article.url} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()}
                    className="p-1.5 rounded-lg text-zinc-400 hover:text-foreground hover:bg-zinc-100 dark:hover:bg-white/10 transition-colors shadow-sm bg-white dark:bg-[#1a1b1e]" title={t("pipeline.openArticle", "Open article")}>
                    <ExternalLink className="w-3.5 h-3.5" />
                  </a>
                )}
                <button onClick={e => { e.stopPropagation(); onDelete(article.id); }}
                  className="p-1.5 rounded-lg text-zinc-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors shadow-sm bg-white dark:bg-[#1a1b1e]" title={t("pipeline.removeArticle", "Remove article")}>
